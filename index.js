...
if (textMsg === "laporan") {
  ...
  try {
    ...
    const perOrang = Math.floor((data.upah || 0) / 4);
...
👷 *Upah Panen*
Rp ${(data.upah || 0).toLocaleString()} Rp ${perOrang.toLocaleString()} / orang
...
const harga = hargaMatch
  ? parseInt(hargaMatch[1])
  : 0;

const penolongMatch = text.match(/penolong\s*(\d+)/i);

const penolong = penolongMatch
